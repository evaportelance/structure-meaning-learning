...
get_score_data <- function(filename){
  ...
  df <- df |>
    ...
    mutate(model = 
case_match(model, "joint"~"Joint-learning", "sem-first"~"Semantics-first", "syn-first"~"Syntax-first", "gold"~ "Gold"))
  return(df)
}

#result_main_dir = "./runs/sem-roles-res"
result_main_dir = "./runs/sem-roles-filtered-res"
...
role_sum_results <- role_results |>
  ...
  summarise(mean_tree = mean(tree_score), sd_tree = sd(tree_score), mean_cap=mean(cap_score), sd_cap=sd(cap_score)) |>
