...
## Data rangling
```{r, echo=FALSE}
#result_main_dir = "./runs/in-dist"
result_main_dir = "./runs/out-dist"
...
get_model_results <- function(dirname){
  ...
  seed_dirs = list.dirs(dirname, recursive = FALSE)
...
plot_data <- sem_boot_test |>
  ...
  mutate(model = 
case_match(model, "joint-model"~"Joint-learning", "sem-first-model"~"Semantics-first", "syn-first-model"~"Syntax-first", "gold-model"~ "Gold")) |>
...
plot_data <- plot_data_intr |>
  mutate(mean_score_intr = mean_score, sd_score_intr = sd_score) |>
  select(-c(mean_score, sd_score, v_type)) |>
  left_join(plot_data_tr) |>
  mutate(mean_score = (mean_score+mean_score_intr)/2, sd_score = (sd_score_intr+sd_score)/2)
